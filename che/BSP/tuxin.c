...
{
	uint32_t length = 0;
	static uint8_t txbuff[200] = {0};
//		//按要求填写帧头
	length = sizeof(show_data.frame_header) + 
		sizeof(show_data.student_interactive_header) + 
			sizeof(show_data.operate_data.show_single.grapic_data_struct) + 
				30 + 
			sizeof(show_data.cmd_id) + 
					sizeof(show_data.frame_tail);
	//length = sizeof(show_data) + 30;
	show_data.frame_header.SOF = Judge_Data_SOF;
